...
open class GenUniModulesIUiXComponentsIPickerIPicker : VueComponent {
    ...
    open var getValues: () -> Any
        get() {
            return unref(this.`$exposed`["getValues"]) as () -> Any
        }
        set(value) {
            setRefValue(this.`$exposed`, "getValues", value)
        }
    open var getColumns: () -> Any
        get() {
            return unref(this.`$exposed`["getColumns"]) as () -> Any
        }
        set(value) {
            setRefValue(this.`$exposed`, "getColumns", value)
        }
    open var getColumnValues: (columnIndex: Number) -> UTSArray<IPickerItem>
        get() {
            return unref(this.`$exposed`["getColumnValues"]) as (columnIndex: Number) -> UTSArray<IPickerItem>
        }
        set(value) {
        ...
        var setup: (__props: GenUniModulesIUiXComponentsIPickerIPicker, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            ...
            fun gen_normalizeItem_fn(item: Any?): IPickerItem {
                if (item != null && UTSAndroid.`typeof`(item) == "object") {
                    ...
                    val kObject = UTSAndroid.consoleDebugError(JSON.parse(serialized), " at uni_modules/i-ui-x/components/i-picker/i-picker.uvue:300") as UTSJSONObject
                    ...
                    return IPickerItem(text = text, value = value, disabled = kObject["disabled"] == true)
                }
                return IPickerItem(text = if (item == null) {
                    ""
                } else {
                    item.toString()
                }
            ...
            fun gen_normalizeColumn_fn(list: Any?): UTSArray<IPickerItem> {
                val result: UTSArray<IPickerItem> = _uA()
                if (list == null || !UTSArray.isArray(list)) {
                    return result
                }
                val values = list as UTSArray<Any?>
                run {
                    var i: Number = 0
            ...
            val normalizedColumns = computed(fun(): UTSArray<UTSArray<IPickerItem>> {
                ...
                val source: UTSArray<Any?> = if (columns != null && columns.length > 0) {
                    columns
                } else {
                    if (options == null) {
                        _uA()
                ...
                val first = source[0]
                if (isArray(first)) {
                    val result: UTSArray<UTSArray<IPickerItem>> = _uA()
                    run {
                        var i: Number = 0
            ...
            fun gen_selectedItems_fn(): UTSArray<IPickerItem> {
                val result: UTSArray<IPickerItem> = _uA()
            ...
            fun gen_columnAt_fn(index: Number): UTSArray<IPickerItem> {
            ...
            fun gen_itemHeightNumber_fn(): Number {
                val height = parseFloat(props.itemHeight.toString())
            ...
            val column0 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(0)
            }
            )
            val column1 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(1)
            }
            )
            val column2 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(2)
            }
            )
            val column3 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(3)
            }
            )
            val column4 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(4)
            }
            )
            val column5 = computed(fun(): UTSArray<IPickerItem> {
                return columnAt(5)
            }
            )
            ...
            fun gen_emitSelectedValue_fn(): Unit {
            ...
            fun gen_itemClass_fn(item: IPickerItem, columnIndex: Number, itemIndex: Number): String {
                val classes = _uA(
                    "i-picker__item"
                )
            ...
            fun gen_itemTextClass_fn(item: IPickerItem, columnIndex: Number, itemIndex: Number): String {
            ...
            fun gen_itemTextStyle_fn(item: IPickerItem, columnIndex: Number, itemIndex: Number): String {
            ...
            syncIndexs()
            __expose(_uM("open" to open, "close" to close, "clear" to clear, "getIndexs" to getIndexs, "getValues" to getValues, "getColumns" to getColumns, "getColumnValues" to getColumnValues, "setColumnValues" to setColumnValues))
            return fun(): Any? {
