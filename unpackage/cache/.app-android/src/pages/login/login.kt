    ...
    companion object {
        ...
        var setup: (__props: GenPagesLoginLogin) -> Any? = fun(__props): Any? {
            ...
            fun gen_loadSavedAccount_fn(): Unit {
                try {
                    ...
                    val account = if (UTSAndroid.`typeof`(rawAccount) == "string") {
                        UTSAndroid.consoleDebugError(JSON.parse(rawAccount as String), " at pages/login/login.uvue:174") as UTSJSONObject
                    } else {
                    ...
                    console.error("加载保存的账号密码失败:", error, " at pages/login/login.uvue:179")
                }
            }
            ...
            val getSystemInfo = fun(): Unit {
                ...
                console.log("设备型号:", deviceModel.value, " at pages/login/login.uvue:222")
            }
                ...
                return wrapUTSPromise(suspend w1@{
                        ...
                        try {
                            ...
                            try {
                                ...
                                console.warn("获取应用版本失败，使用默认版本号:", error, " at pages/login/login.uvue:358")
                            }
            ...
            val submit = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        try {
                            console.log("准备验证表单...", " at pages/login/login.uvue:474")
                            if (!validateForm()) {
                                return@w1
                            }
                            console.log("✅ 表单验证通过", " at pages/login/login.uvue:476")
                            val newFormData: UTSJSONObject = _uO("__\$originalPosition" to UTSSourceMapPosition("newFormData", "pages/login/login.uvue", 479, 10), "username" to form.value.username, "password" to form.value.password, "from" to deviceModel.value, "type" to "USER")
                            console.log("📤 请求参数:", newFormData, " at pages/login/login.uvue:485")
                            loading.value = true
                            uni_showLoading(ShowLoadingOptions(title = "登录中...", mask = true))
                            console.log("🚀 开始调用 login 接口...", " at pages/login/login.uvue:495")
                            val res = await(login(newFormData))
                            console.log("✅ 登录接口返回:", res, " at pages/login/login.uvue:497")
                            loading.value = false
                            ...
                            console.error("❌ 登录失败:", error, " at pages/login/login.uvue:516")
                            loading.value = false
                            uni_hideLoading(null)
            ...
            onMounted(fun(){
                ...
                loadSavedAccount()
                prefetchUniVerify()
                console.log("pswLogin 初始值:", pswLogin.value, " at pages/login/login.uvue:615")
            }
            )
            onUnmounted(fun(){
                stopSmsCooldown()
