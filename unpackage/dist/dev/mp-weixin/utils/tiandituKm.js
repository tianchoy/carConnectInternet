...
class TianMapService {
    ...
    return common_vendor.__awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (straightDistance < this.CONFIG.MIN_DISTANCE_THRESHOLD) {
          common_vendor.index.__f__("log", "at utils/tiandituKm.uts:33", "距离太近，直接使用直线距离:", straightDistance, "米");
        ...
        common_vendor.index.__f__("log", "at utils/tiandituKm.uts:46", "请求天地图URL:", url);
        ...
        if (result.statusCode === 200 && result.data) {
          ...
          if (distance > 0) {
            common_vendor.index.__f__("log", "at utils/tiandituKm.uts:60", `道路距离计算成功: ${startLng},${startLat} -> ${endLng},${endLat} = ${distance}米`);
            return distance;
          } else {
            common_vendor.index.__f__("warn", "at utils/tiandituKm.uts:63", "道路距离计算失败，使用直线距离");
            return straightDistance;
          }
        } else {
          common_vendor.index.__f__("warn", "at utils/tiandituKm.uts:67", `天地图API请求失败，状态码: ${result.statusCode}`);
          if (retryCount < this.CONFIG.MAX_RETRY_ATTEMPTS) {
            common_vendor.index.__f__("log", "at utils/tiandituKm.uts:70", `进行第${retryCount + 1}次重试...`);
        ...
        common_vendor.index.__f__("error", "at utils/tiandituKm.uts:78", "道路规划请求异常:", error);
        if (retryCount < this.CONFIG.MAX_RETRY_ATTEMPTS) {
          common_vendor.index.__f__("log", "at utils/tiandituKm.uts:82", `进行第${retryCount + 1}次重试...`);
          return yield this.getRouteDistance(startLng, startLat, endLng, endLat, {
            retryCount: retryCount + 1
          });
        }
        const straightDistance = this.calculateStraightDistance(startLng, startLat, endLng, endLat);
        common_vendor.index.__f__("warn", "at utils/tiandituKm.uts:89", "所有重试失败，使用直线距离:", straightDistance, "米");
        return straightDistance;
      }
    });
  }
  ...
  static parseDistanceFromXML(xmlString) {
    try {
      ...
      common_vendor.index.__f__("warn", "at utils/tiandituKm.uts:130", "XML中未找到有效的距离信息");
      return 0;
    } catch (error) {
      common_vendor.index.__f__("error", "at utils/tiandituKm.uts:134", "XML解析错误:", error);
      return 0;
    }
  }
  // 计算直线距离（Haversine公式）
  static calculateStraightDistance(lng1, lat1, lng2, lat2) {
    if (!this.isValidCoordinate(lng1, lat1) || !this.isValidCoordinate(lng2, lat2)) {
      common_vendor.index.__f__("error", "at utils/tiandituKm.uts:143", "无效的坐标参数，无法计算距离");
      return 0;
    }
    const R = 6371e3;
      ...
      try {
        for (var routes_1 = common_vendor.__values(routes), routes_1_1 = routes_1.next(); !routes_1_1.done; routes_1_1 = routes_1.next()) {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at utils/tiandituKm.uts:231", `计算路径距离失败: ${route.id || "未知路径"}`, error);
            const straightDistance = this.calculateStraightDistance(route.startLng, route.startLat, route.endLng, route.endLat);
            results.push({
              id: route.id,
              startLng: route.startLng,
...
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/tiandituKm.js.map
