  ...
  setup(__props) {
    ...
    const saveSelectedDevice = (device) => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/index/index.uvue:363", "保存选中设备成功:", deviceInfo);
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.uvue:365", "保存选中设备失败:", error);
      }
    };
    ...
    const getSavedSelectedDevice = () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/index.uvue:423", "获取保存设备失败:", error);
      }
      return null;
    };
    const clearSavedSelectedDevice = () => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/index/index.uvue:432", "清除保存设备成功");
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.uvue:434", "清除保存设备失败:", error);
      }
    };
    const saveSelectedDeviceIndex = (index) => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/index.uvue:443", "保存选中设备索引失败:", error);
      }
    };
    const getSavedSelectedDeviceIndex = () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/index.uvue:456", "获取保存设备索引失败:", error);
      }
      return null;
    };
    const clearSavedSelectedDeviceIndex = () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/index/index.uvue:466", "清除保存设备索引失败:", error);
      }
    };
    const handlePicker = () => {
    ...
    const loadDeviceDetail = (deviceId) => {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          common_vendor.index.__f__("error", "at pages/index/index.uvue:569", "加载设备详情失败", error);
        }
      });
    };
    ...
    const loadTrackPos = (data) => {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          if (res.code != 0) {
            common_vendor.index.__f__("error", "at pages/index/index.uvue:642", "加载轨迹失败:", res.msg);
            clearTripData();
            return Promise.resolve(null);
          }
          ...
          if (requestId != trackRequestId)
            return Promise.resolve(null);
          common_vendor.index.__f__("error", "at pages/index/index.uvue:650", "加载轨迹失败", error);
          clearTripData();
        }
      });
    };
      ...
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          if (res.code != 0 || positions == null || positions.length == 0) {
            common_vendor.index.__f__("warn", "at pages/index/index.uvue:668", "获取设备位置失败:", data.getString("deviceId", ""), res.code);
          ...
          if (!isValidCoordinate) {
            common_vendor.index.__f__("error", "at pages/index/index.uvue:683", "经纬度格式错误", position.getString("latitude", ""), position.getString("longitude", ""));
          ...
          common_vendor.index.__f__("log", "at pages/index/index.uvue:708", "标记点更新完成:", data.getString("deviceId", ""), convertedCoord.lat, convertedCoord.lng);
          return true;
        } catch (error) {
          common_vendor.index.__f__("error", "at pages/index/index.uvue:711", "加载设备位置失败", error);
    ...
    const loadDeviceData = (device) => {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        common_vendor.index.__f__("log", "at pages/index/index.uvue:723", "开始加载设备数据:", device);
        try {
          ...
          common_vendor.index.__f__("error", "at pages/index/index.uvue:736", "切换车辆失败", error);
          utils_toast.showAppToast({
            title: "切换失败，请重试",
            icon: "none"
          });
    ...
    const handleConfirm = (e) => {
      ...
      if (selectedIndex < 0 || selectedIndex >= deviceList.value.length) {
        common_vendor.index.__f__("warn", "at pages/index/index.uvue:758", "无法解析选中的索引，使用当前设备");
        const currentIndex = deviceList.value.findIndex((device) => {
          return device.imei == currentCarImei.value || device.deviceId == currentCarDeviceId.value;
        });
        if (currentIndex != -1) {
          selectedIndex = currentIndex;
          common_vendor.index.__f__("log", "at pages/index/index.uvue:764", "使用当前设备索引:", selectedIndex);
        } else {
          selectedIndex = 0;
          common_vendor.index.__f__("log", "at pages/index/index.uvue:767", "使用默认索引: 0");
        }
      }
      ...
      if (selectedDevice.imei == currentCarImei.value && selectedDevice.deviceId == currentCarDeviceId.value) {
        common_vendor.index.__f__("log", "at pages/index/index.uvue:782", "选择的设备与当前设备相同，不重复加载");
        return null;
      }
        ...
        try {
          ...
          if (code == 0 && list != null && list.length > 0) {
            ...
            if (!selectedDevice && deviceList.value.length > 0) {
              ...
              common_vendor.index.__f__("log", "at pages/index/index.uvue:885", "使用第一个设备作为默认:", selectedDevice === null || selectedDevice === void 0 ? null : selectedDevice.deviceName);
            }
          ...
          common_vendor.index.__f__("error", "at pages/index/index.uvue:923", "加载车辆列表失败", error);
          utils_toast.showAppToast({
            title: "加载失败，请下拉重试",
            icon: "none"
          });
    ...
    const refreshLocation = () => {
      return common_vendor.__awaiter(this, void 0, void 0, function* () {
        ...
        try {
          ...
          common_vendor.index.__f__("error", "at pages/index/index.uvue:952", "刷新位置失败", error);
          utils_toast.showAppToast({
            title: "刷新失败",
            icon: "none"
          });
      ...
      common_vendor.index.navigateTo({
        ...
        fail: (err) => {
          if (err.errMsg.indexOf("locked") < 0)
            common_vendor.index.__f__("error", "at pages/index/index.uvue:984", "跳转轨迹详情失败:", err);
        }
      });
    };
    const toDeviceList = () => {
      common_vendor.index.__f__("log", "at pages/index/index.uvue:991", "toDeviceList");
      if (!isLogin())
        return null;
      common_vendor.index.navigateTo({
    ...
    const toAdd = () => {
      ...
      common_vendor.index.navigateTo({
        ...
        fail: (err) => {
          if (err.errMsg.indexOf("locked") < 0)
            common_vendor.index.__f__("error", "at pages/index/index.uvue:1019", "跳转添加设备失败:", err);
        }
      });
    };
    const toMsgCenter = () => {
    ...
    const toFindCar = () => {
      ...
      common_vendor.index.openLocation({
        ...
        fail: (err) => {
          ...
          common_vendor.index.__f__("error", "at pages/index/index.uvue:1051", "调起地图失败:", err);
        }
      });
    };
    const toFence = () => {
