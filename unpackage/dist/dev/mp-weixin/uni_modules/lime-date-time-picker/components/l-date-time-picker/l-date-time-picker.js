...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props, _a) {
    ...
    const minDate = common_vendor.computed(() => {
      return normalize(props.start, uni_modules_limeDayuts_common_index.dayuts().subtract(1, "year"));
    });
    const maxDate = common_vendor.computed(() => {
      return normalize(props.end, uni_modules_limeDayuts_common_index.dayuts());
    });
    ...
    const calcDate = (currentValue = null) => {
      ...
      if (isTimeMode && typeof currentValue == "string") {
        ...
        const dateStr = uni_modules_limeDayuts_common_index.dayuts(minDate.value).format(format);
        currentValue = `${dateStr}${space}${currentValue}`;
      }
      return currentValue != null && uni_modules_limeDayuts_common_index.dayuts(currentValue).isValid() ? rationalize(uni_modules_limeDayuts_common_index.dayuts(currentValue)) : maxDate.value;
    };
    ...
    const columns = common_vendor.computed(() => {
      ...
      const _b2 = common_vendor.__read(getDate(minDate.value), 6), minYear = _b2[0], minMonth = _b2[1], minDay = _b2[2], minHour = _b2[3], minMinute = _b2[4], minSecond = _b2[5];
      const _c = common_vendor.__read(getDate(maxDate.value), 6), maxYear = _c[0], maxMonth = _c[1], maxDay = _c[2], maxHour = _c[3], maxMinute = _c[4], maxSecond = _c[5];
    ...
    return (_ctx, _cache) => {
      ...
      const __returned__ = {
        ...
        b: common_vendor.o(onConfirm, "67"),
        c: common_vendor.o(onCancel, "c4"),
        d: common_vendor.o(onChange, "21"),
        e: common_vendor.o(onPick, "59"),
        f: common_vendor.p({
          title: _ctx.title,
