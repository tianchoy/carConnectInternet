...
export function openExternalMap(params: ExternalMapNavigationParams): ExternalMapNavigationResult {
	...
	if (activity == null) {
		__f__('error','at uni_modules/external-map-navigation/utssdk/app-android/index.uts:24','获取当前页面失败，无法打开地图')
	...
	try {
		...
		__f__('log','at uni_modules/external-map-navigation/utssdk/app-android/index.uts:39','已请求打开外部地图:', geoUri)
		return result('opened')
	} catch (error) {
		__f__('error','at uni_modules/external-map-navigation/utssdk/app-android/index.uts:42','打开外部地图失败:', error)
		return result('launch_failed')
	}
}
