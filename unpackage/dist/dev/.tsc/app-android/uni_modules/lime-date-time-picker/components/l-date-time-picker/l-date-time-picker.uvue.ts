...
import type { DateTimePickerProps, DateValue, DateTimePickerColumn, TimeModeValues, DateTimePickerColumnItem } from './type';
	...
	import { type DayutsUnit } from '@/uni_modules/lime-dayuts'
...
const _cache = __ins.renderCache;
	 ...
	 * <br> 插件类型：LDateTimePickerComponentPublicInstance 
	 ...
	 * @property {string|number} end 最大可选时间（默认当前时间）
	 * @property {string|number} start 最小可选时间（默认当前时间-5年）
...
__ins.emit(event, ...do_not_transform_spread)
	...
	const isTimeMode = ['hour', 'minute', 'second'].includes(meaningColumn[0]);
	//@ts-ignore
	const normalize = (val : DateValue | null, defaultDay : Dayuts) : Dayuts => val != '' && val != null && dayuts(val).isValid() ? dayuts(val) : defaultDay;
	const minDate = computed(() : Dayuts => normalize(props.start as DateValue | null, dayuts().subtract(1, 'year')));
	const maxDate = computed(() : Dayuts => normalize(props.end as DateValue | null, dayuts()));
	const rationalize = (val : Dayuts) : Dayuts => {
		if (isTimeMode) return val;
		if (val.isBefore(minDate.value)) return minDate.value;
		if (val.isAfter(maxDate.value)) return maxDate.value;
		return val;
	};
	...
	const calcDate = (currentValue : DateValue | null) : Dayuts => {
		...
		// 当首次为空值的时候，会new Date当前时间，若不判断会造成时分时间不为当前时分
	...
	const columns = computed(() : DateTimePickerColumn[] => {
		...
		const [minYear, minMonth, minDay, minHour, minMinute, minSecond] = getDate(minDate.value);
		const [maxYear, maxMonth, maxDay, maxHour, maxMinute, maxSecond] = getDate(maxDate.value);
...
const _component_l_picker = resolveEasyComponent("l-picker",_easycom_l_picker)
  ...
  return _cV(_component_l_picker, _uM({
    ...
    onChange: onChange,
    onPick: onPick
  }), null, 8 /* PROPS */, ["title", "titleStyle", "confirm-btn", "confirm-style", "cancel-btn", "cancel-style", "itemHeight", "itemColor", "itemFontSize", "itemActiveColor", "indicatorStyle", "bgColor", "groupHeight", "radius", "value", "columns", "maskColors"])
}
}
