...
function selectedValue() : any | null {
  const modelValue = props.modelValue
  if (modelValue != null && modelValue.toString().length > 0) return modelValue
  return props.value
}
...
function isChecked() : boolean {
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < list.length; i++) {
      const item = list[i]
      if (item != null && item.toString() == nameText()) return true
    }
    return false
  }
...
function buildValue(nextChecked : boolean, previousChecked : boolean) : any {
  ...
  if (value != null && Array.isArray(value)) {
    const list : Array<any | null> = (value as Array<any | null>).slice(0)
    if (nextChecked && !previousChecked) list.push(props.name)
    if (!nextChecked && previousChecked) {
      const nextList : Array<any | null> = []
      for (let i = 0; i < list.length; i++) {
        const item = list[i]
        if (item == null || item.toString() != nameText()) nextList.push(item)
      }
      return nextList
    }
    return list
...
const wrapClass = computed(() : string => {
  const classes : Array<string> = ['i-checkbox']
...
const labelClass = computed(() : string => {
  const classes : Array<string> = ['i-checkbox__label']
...
const markStyle = computed(() : string => {
