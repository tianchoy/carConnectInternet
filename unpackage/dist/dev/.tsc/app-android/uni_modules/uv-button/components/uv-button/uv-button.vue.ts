...
const __sfc__ = defineComponent({
		...
		computed: {
			...
			baseColor() {
				let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uv-button/components/uv-button/uv-button.vue", 250, 9),};
				if (this.color) {
			...
			nvueTextStyle() {
				let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uv-button/components/uv-button/uv-button.vue", 280, 9),};
				// 针对自定义了color颜色的情况，镂空状态下，就是用自定义的颜色
				if (this.type === "info") {
					style.color = "#323233";
				}
			...
			// 设置外层盒子的宽度，其他样式不需要
			btnWrapperStyle() {
				const style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uv-button/components/uv-button/uv-button.vue", 308, 11),};
