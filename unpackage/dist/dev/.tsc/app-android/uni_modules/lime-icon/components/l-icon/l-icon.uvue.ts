import _easycom_l_svg from '@/uni_modules/lime-svg/components/l-svg/l-svg.uvue'
...
const _component_l_svg = resolveEasyComponent("l-svg",_easycom_l_svg)
  ...
  return isTrue(!unref(isImage) && !unref(isIconify) && !unref(isSVG))
    ...
    : isTrue((!unref(isSVG) && !unref(isIconify)) && unref(isImage))
      ...
      : _cV(_component_l_svg, _uM({
          ...
          onLoad: imageload,
          onClick: () => {_ctx.$emit('click')}
        }), null, 8 /* PROPS */, ["class", "style", "color", "src", "web", "onClick"])
}
}
...
const GenUniModulesLimeIconComponentsLIconLIconStyles = [_uM([["l-icon--font", _pS(_uM([["fontFamily", "l"], ["textAlign", "center"], ["fontSize", "var(--l-icon-size, 32px)"], ["color", "var(--l-icon-color, rgba(0, 0, 0, 0.88))"]]))], ["l-icon--image", _pS(_uM([["width", "var(--l-icon-size, 32px)"], ["height", "var(--l-icon-size, 32px)"]]))], ["@FONT-FACE", _uM([["0", _uM([["fontFamily", "l"], ["src", "url(\"uni_modules/lime-icon/hybrid/html/t3.ttf\")"]])]])]])]
