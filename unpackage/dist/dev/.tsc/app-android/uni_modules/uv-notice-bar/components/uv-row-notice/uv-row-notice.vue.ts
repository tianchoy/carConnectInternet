	...
	const __sfc__ = defineComponent({
		...
		computed: {
			// 文字内容的样式
			textStyle() {
				let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uv-notice-bar/components/uv-row-notice/uv-row-notice.vue", 128, 9),}
				style.color = this.color
				style.fontSize = this.$uv.addUnit(this.fontSize)
				return style
			},
			animationStyle() {
				let style = {__$originalPosition: new UTSSourceMapPosition("style", "uni_modules/uv-notice-bar/components/uv-row-notice/uv-row-notice.vue", 134, 9),}
