...
// Keep UniPush as the production default. The override is deliberately local-storage
// based so test builds can switch provider without changing normal user-facing flows.
const DEFAULT_PUSH_PROVIDER: PushProviderName = 'unipush'
const ENABLE_LOCAL_PROVIDER_SWITCH = true
...
function pushDebug(provider: PushProviderName, message: string): void {
	...
	__f__('error','at services/push.uts:72','[PushManager][' + provider + '] ' + message)
}
...
function selectedPushProvider(): PushProviderName {
	...
	if (!ENABLE_LOCAL_PROVIDER_SWITCH) return DEFAULT_PUSH_PROVIDER
	const override = storageString(PUSH_LOCAL_PROVIDER_OVERRIDE_KEY)
	return override == 'jpush' ? 'jpush' : DEFAULT_PUSH_PROVIDER
	// return 'jpush'
...
class PushManager {
	...
	private saveRegistrationId(registrationId: string): void {
		...
		const registrationIdLabel = this.provider == 'unipush' ? 'UniPush CID' : 'JPush RegistrationID'
...
// Development-only hook. Set this before launching/relaunching the app to test the
// installed JPush SDK; normal builds keep UniPush unless this value is explicitly set.
