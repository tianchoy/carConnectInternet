...
function pushDebug(provider: PushProviderName, message: string): void {
	...
	__f__('error','at services/push.uts:88','[PushManager][' + provider + '] ' + message)
}
...
function notifyPushRegistrationIdReady(registrationId: string): void {
	for (let index = 0; index < pushRegistrationIdReadyListeners.length; index++) {
		try {
			...
			__f__('error','at services/push.uts:96','[PushManager] RegistrationID 就绪监听执行失败:', error)
		}
	}
}
...
function notifyPushSessionAuthenticated(registrationId: string): void {
	for (let index = 0; index < pushSessionAuthenticatedListeners.length; index++) {
		try {
			...
			__f__('error','at services/push.uts:106','[PushManager] 已认证会话监听执行失败:', error)
		}
	}
}
