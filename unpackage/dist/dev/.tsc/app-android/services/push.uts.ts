...
function pushDebug(message: string): void {
	...
	__f__('error','at services/push.uts:23','[UniPushDebug]', message)
}
...
function registerPushListener(): void {
	...
	try {
		uni.onPushMessage((event: any) => {
			__f__('log','at services/push.uts:103','收到 UniPush 消息')
			savePushEvent(event)
		...
		__f__('error','at services/push.uts:110','注册 UniPush 监听失败:', error)
	}
