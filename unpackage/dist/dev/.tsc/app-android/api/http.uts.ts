...
function handleTokenExpired(): void {
    ...
    __f__('log','at api/http.uts:51','检测到token过期，执行跳转登录页逻辑')
    ...
    setTimeout(() => {
        __f__('log','at api/http.uts:66','正在跳转到登录页...')
        uni.redirectTo({
            url: '/pages/login/login',
            success: () => {
                __f__('log','at api/http.uts:70','跳转登录页成功')
            },
            fail: (err) => {
                __f__('log','at api/http.uts:73','跳转登录页失败:', err)
...
function logHttpError(error: HttpError): void {
    ...
    __f__('error','at api/http.uts:123','[HttpRequest] ' + detail)
}
