...
function handleTokenExpired(): void {
    __f__('log','at api/http.uts:38','检测到token过期，执行跳转登录页逻辑')
    ...
    setTimeout(() => {
        __f__('log','at api/http.uts:52','正在跳转到登录页...')
        uni.redirectTo({
            url: '/pages/login/login',
            success: () => {
                __f__('log','at api/http.uts:56','跳转登录页成功')
            },
            fail: (err) => {
                __f__('log','at api/http.uts:59','跳转登录页失败:', err)
...
function errorHandler(error: HttpError, config: RequestOptions): void {
    ...
    __f__('log','at api/http.uts:111','请求错误详情:', error)
