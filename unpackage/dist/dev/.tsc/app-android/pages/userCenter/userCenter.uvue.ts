...
const _cache = __ins.renderCache;
	...
	onShow(() => {
		...
		try {
			...
			console.error('获取版本号失败:', error, " at pages/userCenter/userCenter.uvue:96")
		}
	})
	...
	const contactCustomerService = () => {
		uni.openCustomerServiceChat({
			...
			success(res) {
				console.log(res, " at pages/userCenter/userCenter.uvue:105")
			}
		})
	}
	...
	const loadData = async () => {
		let params = {__$originalPosition: new UTSSourceMapPosition("params", "pages/userCenter/userCenter.uvue", 112, 7),}
	...
	const userInfoDetail = () => {
		if (Login.value) {
			uni.navigateTo({
				url: '/pages/userCenter/userInfo/userInfo?userInfo=' + UTSAndroid.consoleDebugError(encodeURIComponent(JSON.stringify(userInfo.value)), " at pages/userCenter/userCenter.uvue:139")
			})
		} else {
			uni.navigateTo({
	...
	const productMall = () => {
		if (Login.value) {
			// 跳转到网页容器页面
			uni.navigateTo({
				url: '/pages/webview/webview?url=' + UTSAndroid.consoleDebugError(encodeURIComponent('https://shop.zdiot.cn/'), " at pages/userCenter/userCenter.uvue:181")
			})
		} else {
			uni.showToast({
				title: '请先登录',
