...
import {logout} from '../../../api/request.uts'
	type UserInfo = { __$originalPosition?: UTSSourceMapPosition<"UserInfo", "pages/userCenter/userInfo/userInfo.uvue", 54, 7>;
		id : string,
		mobile : string,
		type: number,
		createTime:string,
...
const _cache = __ins.renderCache;
	...
	onLoad((options) => {
		...
		if (options.userInfo) {
			try {
				const parsedInfo = UTSAndroid.consoleDebugError(JSON.parse(UTSAndroid.consoleDebugError(decodeURIComponent(options.userInfo), " at pages/userCenter/userInfo/userInfo.uvue:71")), " at pages/userCenter/userInfo/userInfo.uvue:71") as UTSJSONObject
				userInfo.value = {
				...
				console.log("用户信息:", userInfo.value, " at pages/userCenter/userInfo/userInfo.uvue:78")
			} catch (e) {
				console.error("解析用户信息失败:", e, " at pages/userCenter/userInfo/userInfo.uvue:80")
			}
		}
	})
