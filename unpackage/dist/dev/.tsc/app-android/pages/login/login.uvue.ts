...
import { showAppToast } from '../../utils/toast.uts'
	...
	type FormData = { __$originalPosition?: UTSSourceMapPosition<"FormData", "pages/login/login.uvue", 145, 7>;
		username: string
		password: string
	}
	type SavedAccount = { __$originalPosition?: UTSSourceMapPosition<"SavedAccount", "pages/login/login.uvue", 149, 7>;
		username: string
		password: string
	}
...
const docState = ref(false)
	...
	function loadSavedAccount() : void {
		try {
			...
			const account = typeof rawAccount == 'string' ? UTSAndroid.consoleDebugError(JSON.parse(rawAccount), " at pages/login/login.uvue:174") as UTSJSONObject : rawAccount as UTSJSONObject
...
// 提交登录（账号密码）
	const submit = async () => {
		...
		try {
			...
			// 构建请求参数
			const newFormData = {__$originalPosition: new UTSSourceMapPosition("newFormData", "pages/login/login.uvue", 479, 10),
				username: form.value.username,
				password: form.value.password,
