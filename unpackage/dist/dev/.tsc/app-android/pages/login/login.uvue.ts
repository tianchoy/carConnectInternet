...
const _cache = __ins.renderCache;
	...
	const handleGetPhoneNumber = async (e) => {
		...
		try {
			...
			console.log('微信登录code:', loginRes.code, " at pages/login/login.uvue:336")
			...
			console.error('微信登录失败:', error, " at pages/login/login.uvue:367")
			uni.showToast({
				title: '微信登录失败',
				icon: 'none'
			})
	...
	const submit = () => {
		console.log(docState.value, " at pages/login/login.uvue:378")
		if (!docState.value) {
			uni.showToast({
				title: '请先阅读并同意用户协议',
				icon: 'error'
		...
		if (loginType.value) {
			...
			formRef.value?.validate().then(res => {
				const newFormData = {__$originalPosition: new UTSSourceMapPosition("newFormData", "pages/login/login.uvue", 390, 11),
					...form.value,
			...
			smsformRef.value?.validate().then(res => {
				const params = {__$originalPosition: new UTSSourceMapPosition("params", "pages/login/login.uvue", 419, 11),
