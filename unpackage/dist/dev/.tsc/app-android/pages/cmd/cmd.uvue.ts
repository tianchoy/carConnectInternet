...
type TabItem = { __$originalPosition?: UTSSourceMapPosition<"TabItem", "pages/cmd/cmd.uvue", 201, 6>; name: string, value: string }
...
function parseParamConfigs(schema: string): Array<UTSJSONObject> {
	...
	try {
		const parsed = UTSAndroid.consoleDebugError(JSON.parse(schema), " at pages/cmd/cmd.uvue:359")
...
function openOptionSheet(index: number): void {
	...
	getParamOptions(param).forEach((option: UTSJSONObject): void => {
		const action = new UTSJSONObject(new UTSSourceMapPosition("action", "pages/cmd/cmd.uvue", 423, 9))
...
function buildCommandParams(): UTSJSONObject {
	const params = new UTSJSONObject(new UTSSourceMapPosition("params", "pages/cmd/cmd.uvue", 501, 8))
	for (let index = 0; index < paramConfigs.value.length; index++) {
		const value = getParamValue(index).trim()
...
async function loadHistoryPage(reset: boolean): Promise<void> {
	...
	try {
		...
		const query = new UTSJSONObject(new UTSSourceMapPosition("query", "pages/cmd/cmd.uvue", 514, 9))
...
async function sendSelectedCommand(): Promise<void> {
	...
	const requestData = new UTSJSONObject(new UTSSourceMapPosition("requestData", "pages/cmd/cmd.uvue", 556, 8))
