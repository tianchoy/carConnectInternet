...
function parseParamConfigs(schema: string): Array<UTSJSONObject> {
	...
	try {
		const parsed = UTSAndroid.consoleDebugError(JSON.parse(schema), " at pages/cmd/cmd.uvue:353")
		...
		console.error('解析指令参数配置失败:', error, " at pages/cmd/cmd.uvue:380")
...
function openOptionSheet(index: number): void {
	...
	getParamOptions(param).forEach((option: UTSJSONObject): void => {
		const action = new UTSJSONObject(new UTSSourceMapPosition("action", "pages/cmd/cmd.uvue", 417, 9))
...
async function loadAvailableCommands(): Promise<void> {
	...
	try {
		...
		console.error('加载可用指令失败:', error, " at pages/cmd/cmd.uvue:473")
...
function buildCommandParams(): UTSJSONObject {
	const params = new UTSJSONObject(new UTSSourceMapPosition("params", "pages/cmd/cmd.uvue", 495, 8))
	for (let index = 0; index < paramConfigs.value.length; index++) {
		const value = getParamValue(index).trim()
...
async function loadHistoryPage(reset: boolean): Promise<void> {
	...
	try {
		...
		const query = new UTSJSONObject(new UTSSourceMapPosition("query", "pages/cmd/cmd.uvue", 508, 9))
		...
		console.error('加载指令记录失败:', error, " at pages/cmd/cmd.uvue:524")
...
async function sendSelectedCommand(): Promise<void> {
	...
	const requestData = new UTSJSONObject(new UTSSourceMapPosition("requestData", "pages/cmd/cmd.uvue", 550, 8))
	...
	try {
		...
		console.error('下发指令失败:', error, " at pages/cmd/cmd.uvue:567")
...
async function showCommandDetail(record: UTSJSONObject): Promise<void> {
	...
	try {
		...
		console.error('加载指令详情失败:', error, " at pages/cmd/cmd.uvue:647")
...
async function retryCommand(commandId: string): Promise<void> {
	...
	try {
		...
		console.error('重试下发失败:', error, " at pages/cmd/cmd.uvue:674")
