...
const parseParamConfigs = (details: string | null): Array<UTSJSONObject> => {
	...
	try {
		const parsed = UTSAndroid.consoleDebugError(JSON.parse(details), " at pages/cmd/cmd.uvue:130")
...
const sendCommand = async (): Promise<void> => {
	...
	try {
		...
		const response = await sendCmd({
			...
			password: null,
			cmdData: UTSAndroid.consoleDebugError(encodeURIComponent(cmdData), " at pages/cmd/cmd.uvue:285"),
