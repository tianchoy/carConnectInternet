...
import { showAppToast } from '../../utils/toast.uts'
	...
	type TrackPoint = { __$originalPosition?: UTSSourceMapPosition<"TrackPoint", "pages/playBack/playBack.uvue", 80, 7>;
		latitude : number;
		longitude : number;
		rotation : number;
		deviceTime : string;
	...
	type TrackBounds = { __$originalPosition?: UTSSourceMapPosition<"TrackBounds", "pages/playBack/playBack.uvue", 88, 7>;
		minLat : number;
		maxLat : number;
		minLng : number;
		maxLng : number;
	...
	type MapPolylinePoint = { __$originalPosition?: UTSSourceMapPosition<"MapPolylinePoint", "pages/playBack/playBack.uvue", 95, 7>;
		latitude: number
		longitude: number
	}

	type MpPolylineData = { __$originalPosition?: UTSSourceMapPosition<"MpPolylineData", "pages/playBack/playBack.uvue", 100, 7>;
...
const center = reactive({
	...
	function resolveRouteDateTime(dateStr : string) : string | null {
		...
		try {
			const decoded = (UTSAndroid.consoleDebugError(decodeURIComponent(dateStr), " at pages/playBack/playBack.uvue:204") ?? '').replace(/\+/g, ' ').replace('T', ' ')
	...
	const loadTrackPos = async () => {
		...
		const data = {__$originalPosition: new UTSSourceMapPosition("data", "pages/playBack/playBack.uvue", 658, 9),
			imei: imei.value,
        ...
        _cE("view", _uM({ class: "Datetime-box" }), [
          _cE("view", _uM({ class: "date-box" }), [
            ...
            _cE("view", _uM({
              ...
              _cE("text", _uM({ class: "date-part" }), _tD(getPlaybackDate(startTime.value)) + " " + _tD(getPlaybackClock(startTime.value)), 1 /* TEXT */)
            ], 8 /* PROPS */, ["onClick"]),
            _cV(_component_i_icon, _uM({
              class: "date-arrow",
            ...
            _cE("text", _uM({ class: "date-separator" }), "至"),
            _cE("view", _uM({
              class: "date-time-control",
              onClick: () => {showPicker('end')}
            }), [
              _cE("text", _uM({ class: "date-part" }), _tD(getPlaybackDate(endTime.value)) + " " + _tD(getPlaybackClock(endTime.value)), 1 /* TEXT */)
            ], 8 /* PROPS */, ["onClick"]),
            _cV(_component_i_icon, _uM({
              class: "date-arrow",
              name: "/static/xiangxia.png",
