...
const center = reactive({
	...
	function resolveRouteDateTime(dateStr : string) : string | null {
		...
		try {
			const decoded = (UTSAndroid.consoleDebugError(decodeURIComponent(dateStr), " at pages/playBack/playBack.uvue:201") ?? '').replace(/\+/g, ' ').replace('T', ' ')
			const milliseconds = parseLocalDateTime(decoded)
			return milliseconds == null ? null : formatPlaybackTime(milliseconds)
		} catch (error) {
			console.error('解析回放时间失败:', error, " at pages/playBack/playBack.uvue:205")
			return null
		}
	}
