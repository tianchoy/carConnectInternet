...
import _easycom_uv_radio from '@/uni_modules/uv-radio/components/uv-radio/uv-radio.vue'
import _easycom_uv_grid_item from '@/uni_modules/uv-grid/components/uv-grid-item/uv-grid-item.vue'
import _easycom_uv_radio_group from '@/uni_modules/uv-radio/components/uv-radio-group/uv-radio-group.vue'
...
import { ref, reactive, onMounted, computed, watch } from 'vue'
	...
	type Coordinate = { __$originalPosition?: UTSSourceMapPosition<"Coordinate", "pages/geofencing/geofencing.uvue", 184, 7>;
		latitude : number;
		longitude : number;
	};
	// 地图状态
...
const _cache = __ins.renderCache;
	...
	const showFenceModal = ref<any>(null)
	...
	const deviceList = ref<any[]>([]) // 当前显示的设备列表
	const boundDevices = ref<any[]>([]) // 已绑定设备完整列表
	...
	const loadInitialPosition = async () => {
		...
		try {
			const data = {__$originalPosition: new UTSSourceMapPosition("data", "pages/geofencing/geofencing.uvue", 291, 10), deptId: deptId.value, deviceids: imei.value }
			...
			res.data.forEach(async item => {
				if (item.imei == imei.value) {
					...
					const position = {__$originalPosition: new UTSSourceMapPosition("position", "pages/geofencing/geofencing.uvue", 305, 12),
			...
			console.error('获取初始位置失败:', err, " at pages/geofencing/geofencing.uvue:345")
			uni.showToast({
				title: '获取车辆位置失败',
				icon: 'none'
			})
	...
	const loadGeofenceList = async () => {
		try {
			...
			console.error('加载围栏列表失败:', error, " at pages/geofencing/geofencing.uvue:377")
			uni.showToast({ title: '获取围栏列表失败', icon: 'none' })
	...
	// 解析CIRCLE字符串为圆心和半径
	const parseCircle = (circleStr : string) => {
		...
		try {
			...
			if (isNaN(lat) || isNaN(lng) || isNaN(radius) || radius <= 0) {
				console.error('无效的圆形围栏数据:', circleStr, " at pages/geofencing/geofencing.uvue:502")
				return null
			}
			...
			console.error('解析圆形围栏失败:', error, '数据:', circleStr, " at pages/geofencing/geofencing.uvue:515")
			return null
		}
	}
	...
	// 生成CIRCLE字符串
	const generateCircleString = (center : { latitude : number, longitude : number }, radius : number) => {
	...
	// 调整地图中心到围栏中心位置
	const setMapCenterToFence = (fence : any) => {
		const fenceType = getFenceType(fence)
	...
	// 根据半径计算合适的缩放级别
	const calculateZoomLevelFromRadius = (radius : number) => {
			...
			success: async (res) => {
				if (res.confirm) {
					try {
						...
						console.error('删除围栏失败:', error, " at pages/geofencing/geofencing.uvue:677")
						uni.showToast({ title: '删除失败', icon: 'none' })
					}
				}
			}
	...
	const saveFence = async () => {
		...
		const fenceData = {__$originalPosition: new UTSSourceMapPosition("fenceData", "pages/geofencing/geofencing.uvue", 708, 9),
			name: fenceForm.name,
			area: area,
		...
		try {
			...
			console.error('保存围栏失败:', error, " at pages/geofencing/geofencing.uvue:733")
			uni.showToast({ title: '保存失败', icon: 'none' })
		}
	}
	...
	// 初始化分页状态
	const initPagination = (tabType : 'bind' | 'unbind') => {
		pagination[tabType] = {
			pageNum: 1,
			pageSize: 10,
			hasMore: true,
	...
	const loadBoundDevices = async (fenceId : string) => {
		const { pageNum, pageSize, hasMore } = pagination.bind
		// 没有更多数据时停止加载
		if (!hasMore) return
		...
		try {
			...
			if (res.code === 0) {
				...
				// 判断是否还有更多数据（当前页数据量小于每页条数则无更多）
				pagination.bind.hasMore = dataList.length >= pageSize
				// 页码自增
				pagination.bind.pageNum += 1
			}
		} catch (error) {
			console.error('加载已绑定设备失败:', error, " at pages/geofencing/geofencing.uvue:793")
		} finally {
			// 结束加载状态
	...
	const loadUnboundDevices = async () => {
		const { pageNum, pageSize, hasMore } = pagination.unbind
		// 没有更多数据时停止加载
		if (!hasMore) return
		...
		try {
			...
			console.error('加载未绑定设备失败:', error, " at pages/geofencing/geofencing.uvue:827")
		} finally {
			// 结束加载状态
	...
	const toggleDeviceBinding = async (deviceImei : string, bound : boolean) => {
		loading.value = true
		try {
			const params = {__$originalPosition: new UTSSourceMapPosition("params", "pages/geofencing/geofencing.uvue", 849, 10),
			...
			console.error('设备绑定操作失败:', error, " at pages/geofencing/geofencing.uvue:875")
			uni.showToast({ title: '操作失败', icon: 'none' })
		} finally {
			loading.value = false
		}
...
const _component_uv_radio = resolveEasyComponent("uv-radio",_easycom_uv_radio)
const _component_uv_grid_item = resolveEasyComponent("uv-grid-item",_easycom_uv_grid_item)
const _component_uv_radio_group = resolveEasyComponent("uv-radio-group",_easycom_uv_radio_group)
      ...
      isTrue(isDrawing.value)
        ? _cE("view", _uM({
            ...
            drawingMode.value === 'polygon'
              ? _cE("text", _uM({ key: 0 }), "点击地图添加围栏点,至少需要3个点")
              : _cC("v-if", true),
            drawingMode.value === 'circle'
              ? _cE("text", _uM({ key: 1 }), "点击地图确定圆心，再点一下地图确定半径")
              : _cC("v-if", true)
          ])
        : _cC("v-if", true)
    ]),
    _cV(_component_uv_popup, _uM({
        ...
        isTrue(selectedFence.value)
          ? _cE("view", _uM({
              ...
              _cE("view", _uM({ class: "fence-header" }), [
                _cE("text", _uM({ class: "fence-name" }), _tD(selectedFence.value.name), 1 /* TEXT */),
                _cV(_component_uv_icon, _uM({
                  name: "close",
                  onClick: () => {selectedFence.value = null; showFenceModal.value?.close()}
                }), null, 8 /* PROPS */, ["onClick"])
              ]),
              _cE("view", _uM({ class: "fence-actions" }), [
                _cV(_component_uv_button, _uM({
                  size: "small",
                  onClick: () => {editFence(selectedFence.value)}
                }), _uM({
                ...
                _cV(_component_uv_button, _uM({
                  size: "small",
                  type: "error",
                  onClick: () => {deleteFence(selectedFence.value.id)}
                }), _uM({
                ...
                _cV(_component_uv_button, _uM({
                  size: "small",
                  type: "primary",
                  onClick: () => {showBindDevices(selectedFence.value.id)}
                }), _uM({
    ...
    _cE("view", _uM({ class: "tools-panel" }), [
      isTrue(!isDrawing.value && !selectedFence.value)
        ? _cE("view", _uM({
            ...
            _cE("view", _uM({ class: "mode-title" }), "选择围栏类型:"),
      ...
      _cE("view", _uM({ class: "tool-tag-item" }), [
        ...
        _cV(_component_uv_button, _uM({ onClick: clearDrawing }), _uM({
          default: withSlotCtx((): any[] => [" 重置绘制 "]),
          _: 1 /* STABLE */
        })),
        _cV(_component_uv_button, _uM({ onClick: showFenceList }), _uM({
          default: withSlotCtx((): any[] => [" 围栏列表 "]),
          _: 1 /* STABLE */
        }))
      ]),
      _cE("view", _uM({ class: "status-info" }), [
        _cE("text", null, "围栏类型: " + _tD(drawingMode.value === 'polygon' ? '多边形' : '圆形'), 1 /* TEXT */),
        drawingMode.value === 'polygon'
          ? _cE("text", _uM({ key: 0 }), "顶点数量: " + _tD(points.value.length), 1 /* TEXT */)
          : _cC("v-if", true),
        drawingMode.value === 'circle'
          ? _cE("text", _uM({ key: 1 }), "半径: " + _tD(circleRadius.value.toFixed(2) || 0) + "米", 1 /* TEXT */)
          : _cC("v-if", true)
      ])
    ]),
    _cV(_component_uv_popup, _uM({
      ...
      default: withSlotCtx((): any[] => [
        _cE("view", _uM({ class: "fence-list" }), [
          _cE("view", _uM({ class: "list-header" }), [
            ...
            _cV(_component_uv_icon, _uM({
              name: "close",
              onClick: () => {fencesPopup.value.close()}
            }), null, 8 /* PROPS */, ["onClick"])
          ]),
          _cE("scroll-view", _uM({
            class: "list-content",
      ...
      default: withSlotCtx((): any[] => [
        _cE("view", _uM({ class: "edit-dialog" }), [
          _cE("view", _uM({ class: "dialog-header" }), [
            _cE("text", null, _tD(editingFence.value ? '编辑围栏' : '新增围栏'), 1 /* TEXT */)
          ]),
          _cE("view", _uM({ class: "dialog-content" }), [
            ...
            _cE("view", _uM({ class: "radio-group" }), [
              ...
              _cV(_component_uv_grid, _uM({ col: 2 }), _uM({
                default: withSlotCtx((): any[] => [
                  _cV(_component_uv_radio_group, _uM({
                    modelValue: fenceForm.alarmType,
                    "onUpdate:modelValue": $event => {(fenceForm.alarmType) = $event},
                    iconPlacement: "left"
                  }), _uM({
                    default: withSlotCtx((): any[] => [
                      _cV(_component_uv_grid_item, null, _uM({
                        default: withSlotCtx((): any[] => [
                          _cV(_component_uv_radio, _uM({ name: "0" }), _uM({
                      ...
                      _cV(_component_uv_grid_item, null, _uM({
                        default: withSlotCtx((): any[] => [
                          _cV(_component_uv_radio, _uM({ name: "1" }), _uM({
                      ...
                      _cV(_component_uv_grid_item, null, _uM({
                        default: withSlotCtx((): any[] => [
                          _cV(_component_uv_radio, _uM({ name: "2" }), _uM({
                      ...
                      _cV(_component_uv_grid_item, null, _uM({
                        default: withSlotCtx((): any[] => [
                          _cV(_component_uv_radio, _uM({ name: "3" }), _uM({
          ...
          _cE("view", _uM({ class: "dialog-actions" }), [
            _cV(_component_uv_button, _uM({
              onClick: () => {editDialogPopup.value.close()}
            }), _uM({
    ...
    _cV(_component_uv_popup, _uM({
      ...
      default: withSlotCtx((): any[] => [
        _cE("view", _uM({ class: "device-dialog" }), [
          _cE("view", _uM({ class: "dialog-header" }), [
            _cE("text", null, "设备绑定 - " + _tD(currentFenceName.value), 1 /* TEXT */),
            _cV(_component_uv_icon, _uM({
              name: "close",
              onClick: () => {deviceDialogPopup.value.close()}
            }), null, 8 /* PROPS */, ["onClick"])
          ]),
          _cE("view", _uM({ class: "dialog-tabs" }), [
            _cE("view", _uM({
              class: _nC(['tab', activeTab.value === 'bind' ? 'active' : '']),
              onClick: () => {activeTab.value = 'bind'}
            }), " 已绑定设备 ", 10 /* CLASS, PROPS */, ["onClick"]),
            _cE("view", _uM({
              class: _nC(['tab', activeTab.value === 'unbind' ? 'active' : '']),
              onClick: () => {activeTab.value = 'unbind'}
          ...
          _cE("scroll-view", _uM({
            ...
            _cE(Fragment, null, RenderHelpers.renderList(deviceList.value, (device, __key, __index, _cached): any => {
              return _cE("view", _uM({
                ...
                _cE("view", _uM({ class: "device-info" }), [
                  _cE("text", _uM({ class: "name" }), _tD(device.plateNo || device.imei), 1 /* TEXT */),
                  isTrue(device.connectionStatus)
                    ? _cE("text", _uM({
                        key: 0,
                        class: "status"
                      }), _tD(device.connectionStatus === 'online' ? '在线' : '离线'), 1 /* TEXT */)
                    : _cC("v-if", true)
                ]),
                _cV(_component_uv_switch, _uM({
                  "model-value": isDeviceBound(device.imei),
                  onChange: ($event: any) => {toggleDeviceBinding(device.imei, $event)},
            ...
            isTrue(deviceList.value.length === 0 && !loading.value)
              ? _cE("view", _uM({
                  key: 0,
                  class: "empty"
                }), _tD(activeTab.value === 'bind' ? '暂无绑定设备' : '暂无可用设备'), 1 /* TEXT */)
              : _cC("v-if", true),
