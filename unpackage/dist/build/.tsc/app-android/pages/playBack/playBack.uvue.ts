...
const center = reactive({
	...
	const initialStartTime = formatPlaybackTime(Date.now() - 3600000 * 6)
	const startTime = ref(initialStartTime)
	const endTime = ref(initialEndTime)
	const displayStartTime = ref(initialStartTime)
	const displayEndTime = ref(initialEndTime)
      ...
      _cE("view", _uM({ class: "tools-panel" }), [
        _cE("view", _uM({ class: "Datetime-box" }), [
          _cE("view", _uM({ class: "date-box" }), [
            ...
            }), _tD(displayStartTime.value), 9 /* TEXT, PROPS */, ["onClick"]),
            _cE("text", _uM({ class: "date-separator" }), "至"),
            _cE("text", _uM({
              class: "Date",
              onClick: () => {showPicker('end')}
            }), _tD(displayEndTime.value), 9 /* TEXT, PROPS */, ["onClick"])
          ])
        ]),
