...
import _imports_2 from '../../static/car.png'
import _imports_3 from '../../static/dzwl.png'
import _imports_4 from '../../static/msg.png'
import _imports_5 from '../../static/pay.png'
...
const toRecordDetail = () => {
    ...
    const position = devicePosInfo.value
    const latitude = position?.getNumber('latitude', 0) ?? 0
    const longitude = position?.getNumber('longitude', 0) ?? 0
    const hasValidPosition = !isNaN(latitude) && !isNaN(longitude) &&
    ...
    uni.navigateTo({
        url: '/pages/playBack/playBack?imei=' + encodeURIComponent(currentCarImei.value) +
            ...
            '&lat=' + encodeURIComponent(hasValidPosition ? latitude.toString() : '') +
            '&lng=' + encodeURIComponent(hasValidPosition ? longitude.toString() : '') +
            '&startTime=' + encodeURIComponent(formatTimes(timeRange.todayZero)) +
            '&endTime=' + encodeURIComponent(formatTimes(timeRange.nowTime)),
        fail: (err) => {
