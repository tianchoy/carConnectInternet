...
const imei = ref<string>('')
	...
	// 根据两点距离和车辆速度计算动画时长，确保车辆以近似真实速度匀速移动。
	const calculateRealisticAnimationDuration = (distance : number, speedKmh : number) : number => {
		...
		const duration = distance / (validSpeed / 3.6) * 1000
	...
	const startPositionAnimation = (distance : number, speedKmh : number, sessionId : number, done : () => void) => {
	...
	function processAnimationQueue(sessionId : number) : void {
		...
		startPositionAnimation(distance, next.speed, sessionId, () => { if (!isTracking.value || sessionId != trackingSessionId) return; isProcessingQueue.value = false; if (animationQueue.value.length > 0) setTimeout(() => processAnimationQueue(sessionId), 50) })
	}
