...
import VoipDataMessage from 'cn.jpush.android.api.VoipDataMessage';
...
// 导入 EventCallBack 类型
import type { EventCallBack } from '../interface.uts';
...
export class PushMessageReceiver extends JPushMessageReceiver {
    ...
    private convertCmdMessageToJson(cmdMessage: CmdMessage): string {
        try {
            ...
            if (cmdMessage.cmd == 2012) {
                // 小米订阅消息需要向 UTS 层透传结构化的 channel 操作结果
                jsonObject.put("extra", this.convertBundleToJson(cmdMessage.extra));
            } else {
                // 保持历史命令的 extra 序列化行为，避免改变既有回调语义
    ...
    private convertBundleToJson(bundle: Bundle | null): JSONObject {
        try {
            const jsonObject = new JSONObject();
            if (bundle != null) {
                // 使用 ArrayList 来收集 Bundle 的键，避免 iterator() 歧义
                const keySet = bundle.keySet();
                const keyList = new ArrayList<string>(keySet);
                // 在 Kotlin 中 size 是属性，不是方法，所以直接访问
                const size: Int = keyList.size;
                for (let i: Int = 0; i < size; i++) {
                    const key = keyList.get(i);
    ...
    /**
     * VOIP 透传消息回调（小米/荣耀/OPPO/VIVO 厂商通道）
     */
    override onVoipMessage(context: Context, voipDataMessage: VoipDataMessage): void {
